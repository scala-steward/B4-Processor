...
object Instructions {
  ...
  val D_ZFA32Type = Map(
    ...
    "FMVP_D_X" -> BitPat("b101100100001?????000?????1010011")
  )
  val Q_ZFA64Type = Map(
    "FMVH_X_Q" -> BitPat("b111001100001?????000?????1010011"),
    "FMVP_Q_X" -> BitPat("b101101100001?????000?????1010011")
  )
  val ZBT64Type = Map(
  ...
  val ZVFBFWMAType = Map(
    "VFWMACCBF16_VF" -> BitPat("b100011???????????101?????1010111"),
    "VFWMACCBF16_VV" -> BitPat("b100011???????????001?????1010111")
  )
  val ZVKGType = Map(
