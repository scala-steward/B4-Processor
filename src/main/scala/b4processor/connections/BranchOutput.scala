...
object BranchOutput {
  def noResult()(implicit params: Parameters): BranchOutput =
    (new BranchOutput).Lit(_.threadId -> 0.U, _.programCounter -> 0.S)

  def branch(threadId: UInt, address: SInt)(implicit
    params: Parameters
  ): BranchOutput =
    (new BranchOutput).Lit(_.threadId -> threadId, _.programCounter -> address)
}
