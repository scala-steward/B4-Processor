...
class InstructionChecker extends Module {
  ...
  output.instruction := MuxLookup(input.opcode, Instructions.Unknown, Seq(
    // I
    "b0000011".U -> Instructions.Load,
    "b0001111".U -> Mux(input.function3bits(0),
      Instructions.fencei,
      Instructions.fence),
    "b1110011".U -> Mux(input.function3bits.orR,
      Mux(input.function3bits(2),
        Instructions.CsrI,
        Instructions.Csr),
      Mux(input.function7bits(0),
        Instructions.ebreak,
        Instructions.ecall)),
  ...
  output.branch := Mux(output.instruction === Instructions.Branch, MuxLookup(input.function3bits, BranchOperations.Unknown, Seq(
  ...
  output.operationWidth := Mux(output.instruction === Instructions.Load || output.instruction === Instructions.Store,
    MuxLookup(input.function3bits, OperationWidth.Unknown, Seq(
      0.U -> OperationWidth.Byte,
      1.U -> OperationWidth.HalfWord,
      2.U -> OperationWidth.Word,
      3.U -> OperationWidth.DoubleWord,
    )),
    MuxCase(OperationWidth.Unknown, Seq(
  ...
  output.csr := Mux(output.instruction === Instructions.Csr || output.instruction === Instructions.CsrI,
    MuxLookup(input.function3bits(1, 0), CSROperations.Unknown, Seq(
...
object Instructions extends ChiselEnum {
  val lui, auipc, jal, jalr, Branch, Load, Store, ArithmeticImmediate, Arithmetic, fence, fencei, ecall, ebreak, Csr, CsrI, Unknown = Value
}
