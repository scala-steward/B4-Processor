...
class Option_stag extends Module {
  val io = IO(new Bundle{
    ...
    val reorder_dtag = DecoupledIO(UInt(TAG_WIDTH.W))
  ...
  io.r_value := Wire(MuxCase(1.U, Seq(
    (io.before_dtag.valid === 0.U && io.reorder_dtag.valid === 0.U) -> 0.U)))
  ...
  when(io.r_value) { // Rbit = 1
    io.stag := RegNext(Mux(io.reorder_dtag.valid, io.reorder_dtag.bits, io.before_dtag.bits))
