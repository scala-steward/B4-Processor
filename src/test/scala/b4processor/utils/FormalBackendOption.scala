...
trait FormalBackendOption {
  this: ChiselScalatestTester =>

  /** allows us to specify a formal backend like: -Dformal_engine=btormc */
  def DefaultBackend: FormalEngineAnnotation =
    FormalBackendOption.getEngine(scalaTestContext.value.get.configMap)
}
...
object FormalBackendOption {
  ...
  def getEngine(configMap: org.scalatest.ConfigMap): FormalEngineAnnotation = {
    configMap.get("formal_engine") match {
      ...
      case Some("yices2") =>
        throw new RuntimeException("yices is not supported yet")
      case Some("boolector") =>
        throw new RuntimeException("boolector is not supported yet")
      case Some("bitwuzla") => BitwuzlaEngineAnnotation
      case Some(unknown) =>
        throw new RuntimeException(s"Unknown formal engine: $unknown")
      // default
